<p align="center">
  <img src="https://raw.githubusercontent.com/jdhert/jdhert/main/assets/banner.png" width="100%" />
</p>

# 👋 안녕하세요, 백엔드 개발자 박세한입니다.

> **운영 환경에서 발생하는 문제를 분석하고, 더 안정적이고 효율적인 구조로 개선하는 개발자입니다.**

<br>

## 🙋 About Me
- 그룹웨어 시스템 유지보수 및 개선 경험
- **최대 3,300명 사용자 환경 운영 경험**
- Java / Spring 기반 백엔드 개발 중심
- 장애 대응, 성능 개선, 리팩토링 경험

<br>

## 💼 Experience

### (주)코비젼 | CS사업팀  
**2024.06 ~ 현재**

- 그룹웨어 플랫폼 유지보수 및 운영 담당
- 사용자 환경 기반 장애 대응 및 이슈 해결
- 성능 저하 구간 분석 및 쿼리 최적화 수행
- Deadlock 및 트랜잭션 문제 분석 및 개선
- 레거시 코드 구조 개선 및 리팩토링 진행

<br>

## 🚀 Project Experience

개인 프로젝트를 통해 실제 서비스 개발 흐름을 경험하며,  
기능 구현을 넘어 **구조 설계와 운영 관점의 개선까지 고려한 개발**을 지향합니다.

### What I Focused On
- 사용자 흐름을 고려한 API 설계
- 유지보수 관점에서의 코드 구조 개선
- 데이터 모델링 및 쿼리 최적화
- Docker 기반 실행 환경 구성 및 배포 경험

### Projects
- **CS-ChatBot**  
  유지보수 업무 자동화를 위한 챗봇 서비스  
  https://github.com/jdhert/CS-ChatBot  

- **SceneHive**  
  프론트엔드와 백엔드 연동 기반 웹 애플리케이션  
  https://github.com/jdhert/SceneHive  

- **Pet-Public**  
  커뮤니티 기반 서비스 기능 및 데이터 구조 설계  
  https://github.com/jdhert/Pet-Public  

<br>

## 🛠 Tech Stack

### Backend
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring](https://img.shields.io/badge/Spring-6DB33F?style=for-the-badge&logo=spring&logoColor=white)
![Spring Boot](https://img.shields.io/badge/SpringBoot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)

### Frontend
![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=for-the-badge&logo=vuedotjs&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)

### Database
![MSSQL](https://img.shields.io/badge/MSSQL-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)
![MariaDB](https://img.shields.io/badge/MariaDB-003545?style=for-the-badge&logo=mariadb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)

### Infra / Cloud
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![Oracle Cloud](https://img.shields.io/badge/Oracle%20Cloud-F80000?style=for-the-badge&logo=oracle&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

<br>

## 📜 Certificate
- 정보처리기사 (2024.06.18)  
- SQLD (2024.06.21)  
- 리눅스마스터 2급 (2024.06.28)

<br>

## 📫 Contact
- Email: kkuladong12@gmail.com
